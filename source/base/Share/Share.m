...
@interface Share ()<ISSShareViewDelegate>
{
    codeView *m_codeView;
}
@end
...
+ (void)showShareMenuWithTitle:(NSString *)title content:(NSString *)content url:(NSString *)url iconUrl:(NSString *)iconUrl iconPath:(NSString *)iconPath inView:(UIViewController<ISSViewDelegate,ISSShareViewDelegate> *)viewController {
                 ...
                 statusBarTips:NO
                        result:^(ShareType type, SSResponseState state, id<ISSPlatformShareInfo> statusInfo, id<ICMErrorInfo> error, BOOL end) {
                            if (type==ShareTypeAny) {
                                if (end) {
                                    codeView *view=[[codeView alloc]initWithFrame:CGRectMake(0, 0, kMainScreen_Width, kMainScreen_Height)];
                                    [view codeViewSetUrl:url];
    ...
    id<ISSShareActionSheetItem> pyqItem = [ShareSDK shareActionSheetItemWithTitle:@"微信朋友圈"
                                                                                icon:[UIImage imageNamed:@"share_pyq.png"]
                                                                        clickHandler:^{
                                                                            doShareBlock(ShareTypeWeixiTimeline);
                                                                        }];
    id<ISSShareActionSheetItem> ewmItem = [ShareSDK shareActionSheetItemWithTitle:@"二维码"
                                                                             icon:[UIImage imageNamed:@"二维码"]
                                                                     clickHandler:^{
                                                                         doShareBlock(ShareTypeAny);
                                                                     }];
