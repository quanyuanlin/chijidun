...
@end
@implementation TeamFoodDetailView
...
-(void)addMainViewWith:(CGRect)frame
{
    ...
    [m_imgView setImage:[UIImage imageNamed:@"food_nopicture"]];
    [m_imgView loadImage:self.itemTable.img placeHolder:@"food_nopicture"];
...
-(void)loadDetailData
{
    ItemGetDetailRequest *request = [ItemGetDetailRequest new];
    request.Id = self.itemTable.Id;
    [self.cgClient hideProgress];
    [self.cgClient doItemGetDetail:request success:^(CGResponse *data, NSString *url) {
        ItemGetDetailResponse *response=[[ItemGetDetailResponse alloc]initWithCGResponse:data];
        self.itemTable.img=response.item.img;
        self.itemTable.item_desc=response.item.item_desc;
        self.itemTable.material=response.item.material;
        [m_imgView loadImage:self.itemTable.img placeHolder:@"food_nopicture"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
        ...
        _cell.chef=_chef;
        [_cell bindWithTeam:self.itemTable];
        
        __weak __typeof(&*_cell)weakCell =_cell;
        __block __typeof(self)bself = self;
        _cell.teamCartBlock=^(ChefItemTable *item,BOOL animated){
            if (animated) {
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (yOffset > 0) {
        ...
        [m_backBtn setHidden:YES];
    } else {
        [m_backBtn setHidden:NO];
        [m_headerView setHidden:YES];
