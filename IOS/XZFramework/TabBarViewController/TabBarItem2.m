...
@property UILabel *unselectedItem;
@property UILabel *selectedItem;
...
- (void)drawRect:(CGRect)rect {
    ...
    if ([self isSelected]) {
        ...
        item = [self unselectedItem];
        backgroundImage = [self unselectedBackgroundImage];
        titleAttributes = [self unselectedTitleAttributes];
    }
    [item setFont:[UIFont fontWithName:@"iconfont" size:30]];
//    imageSize.width=30;
//    imageSize.height=30;
    
    imageSize=[item.text sizeWithFont:[UIFont fontWithName:@"iconfont" size:30]
                 constrainedToSize:CGSizeMake(frameSize.width, 30)];
    ...
    if (![_title length]) {
        [item drawTextInRect:CGRectMake(roundf(frameSize.width / 2 - imageSize.width / 2) +
    ...
    if ([[self badgeValue] length]) {
        ...
        CGRect badgeBackgroundFrame = CGRectMake(roundf(frameSize.width / 2 + (imageSize.width / 2) * 0.9) +
...
- (UILabel *)finishedSelectedItem {
    return [self selectedItem];
}

- (UILabel *)finishedUnselectedItem {
    return [self unselectedItem];
}

- (void)setFinishedSelectedItem:(UILabel *)selectedItem withFinishedUnselectedItem:(UILabel *)unselectedItem {
    if (selectedItem && (selectedItem != [self selectedItem])) {
        [self setSelectedItem:selectedItem];
    }
    
    if (unselectedItem && (unselectedItem != [self unselectedItem])) {
        [self setUnselectedItem:unselectedItem];
    }
}
